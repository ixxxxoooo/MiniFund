  ...
  return (
    <div className="flex h-full flex-col gap-[var(--size-gap)] bg-[var(--surface)] p-[var(--size-padding)]">
      ...
      <div className="flex items-center justify-between">
        <span className="text-[length:var(--size-font-xs)] font-semibold text-[var(--fg)]">
          {zhCN.tray.panelTitle}
        </span>
        <span className="text-2xs text-[var(--fg-muted)]">{phaseLabel}</span>
      </div>
      ...
      <div className="rounded-[var(--radius-panel)] bg-[var(--surface-secondary)] p-[var(--size-padding)]">
        ...
        <div className="mt-1 flex items-baseline gap-2">
          <span
            className={cn(
              "quote-num text-xl font-semibold",
              stealth
                ? "text-[var(--quote-flat)]"
                : todayProfit > 0
                  ? "text-[var(--quote-up)]"
          ...
          <QuoteText
            ...
            neutral={stealth}
            className="text-[length:var(--size-font-xs)]"
          />
        </div>
        {/* 指数速览 */}
        {indexes.length > 0 && (
          <div className="mt-2 flex items-center gap-3 border-t border-[var(--border-subtle)] pt-2">
            {indexes.slice(0, 3).map((q) => (
              <div key={q.symbol} className="flex items-center gap-1 text-2xs">
          ...
          items.map((item) => {
            ...
            return (
              ...
              >
                ...
                {est?.hasEstimate ? (
                  <QuoteText value={est.estimateGrowth} neutral={stealth} className="shrink-0 text-2xs" />
                ) : (
